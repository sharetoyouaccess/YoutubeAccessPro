...
```
YoutubeAccessPro-2026.09.02.nvda-addon   (in the parent folder, ~111.7 MB)
```

**Current publishing status as of 2026-09-02** (read this before following the steps below):

- The GitHub repo is public and has **four commits pushed to `origin/main`** so far:
  ...
  **A fifth commit for this `2026.09.02` build has NOT been made or pushed yet** - the working tree here has real uncommitted changes (`git status`) beyond `d0eb41e`: confirmed compatibility with NVDA 2026.2 (released 2026-08-31) and bumped `lastTestedNVDAVersion` from `2026.1` to `2026.2` in `manifest.ini`, plus the matching doc/changelog/version updates. No functional code changes were needed - NVDA 2026.2's developer changelog was reviewed and none of its deprecations or API changes (speechDictHandler constants, UIA additions, browseMode changes) are used by this add-on. Commit and push before relying on any of the steps below:
  ```
  git add .
  git commit -m "2026.09.02: confirm NVDA 2026.2 compatibility, bump lastTestedNVDAVersion"
  git push origin main
  ```
- GitHub Releases that already exist: `v2026.07.19`, `v2026.08.18`, `v2026.08.20`, and `v2026.08.25` (all published, all with their `.nvda-addon` asset attached correctly). Leave all four in place as history - **do not edit or delete them.**
- NVDA Add-on Store submissions filed so far:
  ...
  - **issue #11101** - the `v2026.08.25` update (submitted; check current status before assuming it has merged)

  **A new issue for the `v2026.09.02` update has NOT been filed yet.** File it only after the release below exists.
...
(Already done for the first four versions - see the status note above. For this and future updates, just commit and push as shown in that note.)
...
2. Tag: `v2026.09.02` (matches the `version` field in `manifest.ini`). Leave the existing `v2026.07.19`, `v2026.08.18`, `v2026.08.20`, and `v2026.08.25` releases/tags in place as history - create this as a new, separate release, not an edit of any of those.
3. **Before creating the tag**, make sure "Target" is set to `main` and that the `2026.09.02` commit (step above) has actually been pushed - creating a version-named tag against stale source would be misleading.
4. Title: `YouTube Access Pro 2026.09.02`.
5. Attach the file `YoutubeAccessPro-2026.09.02.nvda-addon` (the one in the parent folder, not this one - and make sure it is the latest build, not an earlier same-named or differently-named build from an older round; see this repo's `DEV_NOTES.md` for the round history if unsure which file is current) as a release asset by dragging it into the release form. 111 MB is well under GitHub's 2 GB per-file release asset limit, so no Git LFS is needed for this - LFS only matters for files committed to the git history itself, and this file is deliberately never committed.
6. Publish the release.
7. Copy the asset's direct download link. It will look like:
   ```
   https://github.com/sharetoyouaccess/YoutubeAccessPro/releases/download/v2026.09.02/YoutubeAccessPro-2026.09.02.nvda-addon
   ```
...
2. Fill it out with:
   ...
   | Field | Value |
   |---|---|
   | Download URL | `https://github.com/sharetoyouaccess/YoutubeAccessPro/releases/download/v2026.09.02/YoutubeAccessPro-2026.09.02.nvda-addon` |
...
4. Automated checks run against the pull request (VirusTotal scan of the binary, manifest validation, URL validation). If the same VirIT/Marburg false positive shows up again (it did for all four prior submissions), that alone should not block a routine update the way it can gate a first-time submitter approval - but it's still worth watching the issue for an actual NV Access comment either way.
...
- **`minimumNVDAVersion` / `lastTestedNVDAVersion`**: now `2025.3` and `2026.2` in `manifest.ini` (the `lastTestedNVDAVersion` was bumped from `2026.1` for this release, to confirm compatibility with NVDA 2026.2, released 2026-08-31). Both values are confirmed present in the Store's `nvdaAPIVersions.json` (https://github.com/nvaccess/addon-datastore/blob/master/transform/nvdaAPIVersions.json) as of this check - re-verify if either value changes again in the future.
- **`url` field in `manifest.ini`**: present (`url = "https://github.com/sharetoyouaccess/YoutubeAccessPro"`) and baked into every build produced by `scripts/build_addon.py` from round 21 onward, including this `2026.09.02` file.
- You are not required to have a Windows/NVDA machine to complete steps 1-4 above; nothing in this process runs the add-on. That said, this update makes no functional code changes (only a metadata/compatibility confirmation), so there is no new behavior that specifically needs real-machine testing before publishing - unlike the `2026.08.25` MP4 fix, which did.
